...
# import methods from randomsample.py
...
if args.samplesize is not None: sample_size = args.samplesize
if args.threads is not None: threads = args.threads
if args.begintimestamp is not None: begin_timestamp = args.begintimestamp
if args.endtimestamp is not None: end_timestamp = args.endtimestamp
...
if args.minvideos is not None: min_videos = args.minvideos
...
def main():
    ...
    # keep sampling until minvideos reached
    while min_videos == 0 or total_hits < min_videos:
        ...
        with open(os.path.join(ROOT_DIR, "collections", collection, "queries",
                               f"{random_timestamp}_queries.json"), "w") as f:
            json.dump(ids, f)

        results = []
        with tqdm(total=len(ids)) as pbar:
            with ThreadPoolExecutor(max_workers=threads) as executor:
                futures = [executor.submit(check_url,
                           f"https://www.tiktok.com/@/video/{i}") for i in ids]
        ...
        with open(os.path.join(ROOT_DIR, "collections", collection, "queries",
                               f"{random_timestamp}_hits.json"), "w") as f:
            json.dump(results, f)
